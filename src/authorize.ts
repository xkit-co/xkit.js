...
import { onWindowClose, captureMessages, hasOwnProperty, logger } from './util'
import Emitter from './emitter'

// TODO: keep this in sync with loading.html
const POPUP_READY_MESSAGE = 'authWindow:ready'

interface AuthorizationToBeSetup
  extends Required<Omit<Authorization, 'access_token' | 'status'>> {
...
function isMessageError(msg: unknown): msg is ErrorMessage {
  if (typeof msg === 'object' && msg !== null && hasOwnProperty(msg, 'error')) {
...
const AUTH_POP_PARAMS: string = Object.entries({
  ...
  (paramStr: string, [key, val]: [string, string | number]) =>
    `${paramStr},${key}=${val}`,
  ''
)
...
function monitorAuthWindowReady(config: IKitConfig): () => Promise<void> {
  ...
  const listener = (event: MessageEvent): void => {
    if (
      event.origin === popupOrigin(config) &&
...
function isAuthorizationReadyForSetup(
  auth: Authorization
): auth is AuthorizationToBeSetup {
  return (
    auth.status === AuthorizationStatus.awaiting_callback &&
    Boolean(auth.authorize_url)
  )
}

async function replaceAuthWindowURL(
  config: IKitConfig,
...
export async function prepareAuthWindow<T>(
  config: IKitConfig,
  authWindowCallback: AuthWindowCallback<T>
): Promise<T> {
  ...
  const errors = captureMessages<ErrorMessage>(
    popupHost(config),
    isMessageError
  )
...
export async function prepareAuthWindowWithConfig<T>(
  callWithConfig: CallWithConfig,
  callback: AuthWindowCallback<T>
): Promise<T> {
  return await callWithConfig(
    async (config) => await prepareAuthWindow(config, callback)
  )
}

async function loadAuthWindow(
  callWithConfig: CallWithConfig,
  authWindow: AuthWindow,
  authorization: Authorization
): Promise<void> {
  ...
  callWithConfig(
    async (config) =>
      await replaceAuthWindowURL(
        config,
        authWindow,
        authorization.authorize_url
      )
  ).catch((e) =>
    logger.error(`Not able to replace Auth Window URL: ${e.message as string}`)
  )

  await onAuthWindowClose(authWindow)

  const newAuthorization = await callWithConfig(
    async (config) => await updateAuthorization(config, authorization)
  )
...
async function updateAuthorization(
  ...
  const newAuthorization = await getAuthorization(
    config,
    authorization.authorizer.prototype.slug,
    authorization.id
  )

  if (newAuthorization.status === AuthorizationStatus.error) {
    throw new AuthorizationError(
      newAuthorization.error_message ?? 'Connection failed.',
      newAuthorization.error_code
    )
  }
...
async function loginToAuthWindow(
  callWithConfig: CallWithConfig,
  authWindow: AuthWindow,
  authorization: Authorization
): Promise<AuthWindow> {
  ...
  return await new Promise((resolve, reject) => {
    loginToAuthWindow(callWithConfig, authWindow, authorization)
      ...
      .then(([emitter, status]: [Emitter, AuthorizationStatus]) => {
        ...
        emitter.on<{ status: AuthorizationStatus }>(
          'status_update',
          ({ status }) => {
            ...
            if (isComplete(status)) {
              callWithConfig(
                async (config) =>
                  await updateAuthorization(config, authorization)
              )
                .then(resolve)
                .catch(reject)
        ...
        if (status === AuthorizationStatus.awaiting_callback) {
          loadAuthWindow(callWithConfig, authWindow, authorization).catch(
            reject
          )
        }
      })

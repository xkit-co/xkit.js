...
import { hasOwnProperty, logger } from '../util'
...
function convertLegacyQuery(query: LegacyConnectionQuery): ConnectionQuery {
  if (isLegacySlugQuery(query)) {
    logger.warn(
      'Specifying a connection by its connector slug (e.g. "github") is deprecated. Use an object instead (e.g. `{ slug: "github" }`'
    )
    return { slug: query }
  }
...
export async function getConnection(
  ...
  legacyQuery: LegacyConnectionQuery
): Promise<Connection> {
  const { connection } = await request(config, {
...
export async function getConnectionOrConnector(
  config: AuthorizedConfig,
...
export async function listConnections(
  ...
  const path =
    connectorSlug != null
      ? `/connections?slug=${encodeURIComponent(connectorSlug)}`
      : '/connections'

  const { connections } = await request(config, {
    path,
    method: 'GET'
  })
